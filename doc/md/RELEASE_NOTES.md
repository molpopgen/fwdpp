...
* Namespace KTfwd::experimental has been added as a safe place for experimenting with future library features.  The source files are in fwdpp/experimental.  Currently, these headers are _not_ installed along with the library, because I expect that these files will come and go as the library evolves.  Futher, the code is unlikely to be fully-commented and it will not be processed by doxygen.  Therefore will not be part of the library reference manual.  Example programs depending on them are in experimental_examples.  These examples are "bleeding edge", and are not guaranteed to compile on all systems, especially those without complete working boost installations (including the compiled run-time libraries).
