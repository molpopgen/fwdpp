...
int main(int argc, char ** argv)
{
  ...
  const std::vector<double> mu(K,theta/double(4*N*K));   //per-gamete mutation rate per locus
  ...
  */
  ...
  for(unsigned i=0;i<K;++i)
    {
      recpols.push_back(std::bind(KTfwd::poisson_xover(),r.get(),littler,double(i),double(i)+1.0,
				  std::placeholders::_1,std::placeholders::_2,std::placeholders::_3));
      mmodels.push_back(std::bind(KTfwd::infsites(),
				  std::placeholders::_1,std::placeholders::_2,
				  r.get(),std::ref(pop.mut_lookup),&generation,
				  mu[i],0.,[&r,i](){return gsl_ran_flat(r.get(),double(i),double(i)+1.0);},
				  [](){return 0.;},[](){return 0.;}));
    }
