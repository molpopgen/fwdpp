...
int main(int argc, char ** argv)
{
  ...
  if (argc != 8 )
    {
      std::cerr << "Incorrect number of arguments.\n"
		...
		<< "theta = 4Nu, the scaled neutral mutation rate,summed over all loci\n"
		<< "rho = 4Nr, the scale recombination rate, summed over all loci\n"
		<< "rbw = the recombination rate between locus i and i+1, constant for all i.\n"
  ...
  std::vector<double> locus_weights(K,1.0); //all loci will have equal weight for both mutation and recombination
  std::vector<KTfwd::extensions::shmodel> shmodels(K,KTfwd::extensions::shmodel(KTfwd::extensions::constant(-0.1),
										KTfwd::extensions::constant(1.0)));
  KTfwd::extensions::discrete_mut_model mmodels(locus_starts,
						locus_ends,
						locus_weights,
						locus_starts,
						locus_ends,
  ...
  KTfwd::extensions::discrete_rec_model recmap(locus_starts,locus_ends,locus_weights);

  //Now, synthesize a function that binds to the operator() and recmap

  const auto recpolicy = KTfwd::extensions::bind_drm(recmap,pop.gametes,pop.mutations,
						     r.get(),double(K)*recrate_region + double(K-1)*rbw);
  for( generation = 0; generation < ngens; ++generation )
    {
      ...
      KTfwd::sample_diploid(r.get(),
			    ...
			    //This is the synthesized function bound to operator() of mmodels:
			    KTfwd::extensions::bind_dmm(mmodels,pop.mutations,
							pop.mut_lookup,
							r.get(),
      ...
      assert(popdata_sane(pop.diploids,pop.gametes,pop.mutations,pop.mcounts));
    }
}
