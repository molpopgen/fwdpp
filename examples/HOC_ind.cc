...
struct HOChap
{
               ...
               poptype::lookup_table_t &lookup, const double &sigmu) const
    {
        ...
        while (lookup.find(pos)
               != lookup.end()) // make sure it doesn't exist in the population
            {
                pos = gsl_rng_uniform(r); // if it does, generate a new one
            }
        lookup.insert(pos);
        double E
            = gsl_ran_gaussian(r, sigmu); // effect size of hap, after mutation
        double sum
            = std::accumulate(g.smutations.begin(), g.smutations.end(), 0.,
                              [&mutations](double &d, const std::size_t m) {
                                  return d + mutations[m].s;
                              });
        double esize
...
double
addEsizes(const poptype::gamete_t &h, const poptype::mcont_t &mutations)
{
...
double
gaussianFitness(const double &a, const double &b)
{
    return std::exp(-1. * std::pow(a + b, 2.) / 2.);
}
    ...
    if (argc != 8)
        {
            std::cerr << "Too few arguments\n"
                      << "Usage: " << argv[0]
                      << " N mu sigmu rho ngens nreps seed\n"
                      << "where:\n"
                      << "N = diploid population size\n"
                      << "mu = mutation rate to variants affecting fitness\n"
                      << "sigmu = standard deviation of effect sizes.  E ~ "
                         "N(0,sigmu^2)\n"
                      << "4Nr = population scaledrecombination rate (per "
    ...
    while (nreps--)
        {
            ...
            pop.mutations.reserve(
                std::ceil(std::log(2 * N) * (4. * double(N) * mu)
                          + 0.667 * (4. * double(N) * mu)));
            ...
            for (unsigned generation = 0; generation < ngens; ++generation)
                {
                    ...
                    double wbar = KTfwd::sample_diploid(
                        ...
                        /*
                          ...
                          The _1 is a placeholder for a non-const reference to
                          a gamete (see defn'
                          of HOChap above).
                        */
            ...
            // accumulator library, which
            // we don't use here to reduce dependencies.
            std::vector<double> G;
            for (const auto &d : pop.diploids)
                {
                    double sum = 0.;
                    for (const auto &m : pop.gametes[d.first].smutations)
                        sum += pop.mutations[m].s;
                    for (const auto &m : pop.gametes[d.second].smutations)
                        sum += pop.mutations[m].s;
                    G.push_back(sum);
                }
