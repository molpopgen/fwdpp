...
size_t
migpop(const size_t &source_pop, gsl_rng *r, const double &mig_prob)
{
...
main(int argc, char **argv)
{
    if (argc != 14)
        {
            std::cerr << "Too few arguments.\n"
                      << "Usage: " << argv[0]
                      << " N theta_neutral theta_deleterious rho M s h f1 f2 "
                         "ngens ngens2 nsam seed\n";
            exit(0);
        }
    int argn = 1;
    ...
    std::pair<std::vector<std::pair<double, std::string>>,
        ...
        sample1 = ms_sample_separate(r, pop.mutations, pop.gametes,
                                     pop.diploids[0], n),
