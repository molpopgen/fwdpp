...
/*!
  Semantically identical to standard diploid type, which is pair<size_t,size_t>,
  but this forces fwdpp to assume that it is a custom type, which we exploit for 
  testing.
 */
{
  ...
  second_type second;
  unsigned i;
  custom_diploid_testing_t() : first(first_type()),second(second_type()),i(std::numeric_limits<unsigned>::max()) {}
  custom_diploid_testing_t(first_type g1, first_type g2) : first(g1),second(g2),i(std::numeric_limits<unsigned>::max()) {}
  bool operator==(const custom_diploid_testing_t & rhs) const
  {
