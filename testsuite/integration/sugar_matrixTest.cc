	...
	while(ntests++<1000)
        {
            ...
            // Check that no variants are fixed
            for (auto &&si : s.first)
                {
                    BOOST_REQUIRE(
                        std::count(si.second.begin(), si.second.end(), '1')
                        < 2 * indlist.size());
                }
            ...
            auto v = gsl_matrix_short_const_view_array(
                m.neutral.data(), m.nrow, m.neutral_positions.size());
