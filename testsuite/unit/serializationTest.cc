...
/* The low-level function object fwdpp::io::scalar_writer
 ...
 */
{
    ...
    BOOST_REQUIRE_THROW(fwdpp::io::scalar_writer()(foo, &x),
                        std::runtime_error);

    std::ostringstream foo2;
    BOOST_REQUIRE_NO_THROW(fwdpp::io::scalar_writer()(foo2, &x));
    // Should also throw when failbit, etc., is true
    foo2.setstate(std::ios::failbit);
    BOOST_REQUIRE_THROW(fwdpp::io::scalar_writer()(foo2, &x),
                        std::runtime_error);

    gzFile gzf = nullptr;
    // Will throw on an invalid gzfile
    BOOST_REQUIRE_THROW(fwdpp::io::scalar_writer()(gzf, &x),
                        std::runtime_error);
    gzf = gzopen("test_scalar_writer_exceptions.gz", "wb");
    // Will throw if you attempt to write 0 bytes!!
    BOOST_REQUIRE_THROW(fwdpp::io::scalar_writer()(gzf, &x, 0),
                        std::runtime_error);
    gzclose(gzf);
    // Throw when writing to closed file
    // BOOST_REQUIRE_THROW(fwdpp::io::scalar_writer()(gzf,&x),std::runtime_error);
