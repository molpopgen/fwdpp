...
BOOST_AUTO_TEST_CASE( low_level_copy_deme_test )
{
  ...
  auto rv = KTfwd::copy_deme(mutations,mcounts,gametes,diploids,0); //append a copy of deme 0 to the metapop
  ...
  for( const auto & dips : diploids )
    {
      BOOST_REQUIRE( KTfwd::popdata_sane(dips,gametes,mutations,mcounts) );
    }
}
...
BOOST_AUTO_TEST_CASE( low_level_copy_deme_test_out_of_range )
{
  diploids.emplace_back(dipvector_t(1000,std::make_pair(0,0)));
  gametes.emplace_back(KTfwd::gamete(2000));
  auto rv = KTfwd::copy_deme(mutations,mcounts,gametes,diploids,1); //Now, 1 is out of range, so we'll get an error
  BOOST_REQUIRE(rv != 0);
}
...
BOOST_AUTO_TEST_CASE( low_level_merge_deme_test )
{
  //Two-deme case
  {
    diploids = {dipvector_t(1000,std::make_pair(0,0)),dipvector_t(500,std::make_pair(1,1))};
...
BOOST_AUTO_TEST_CASE( low_level_swap_demes_test )
{
  ...
  
  {
    ...
    gametes={KTfwd::gamete(2000),
	     ...
	     KTfwd::gamete(8000),
	     KTfwd::gamete(10000)};
    auto rv = KTfwd::swap_demes(diploids,1,1); //i==j, so nothing will happen
    BOOST_REQUIRE_EQUAL(rv,1);
    BOOST_REQUIRE(KTfwd::check_sum(gametes,30000));
  ...
  
  {
    ...
    gametes={KTfwd::gamete(2000),
	     ...
	     KTfwd::gamete(8000),
	     KTfwd::gamete(10000)};
    auto rv = KTfwd::swap_demes(diploids,0,5); //deme index j out of range
    BOOST_REQUIRE_EQUAL(rv,-1);
    BOOST_REQUIRE(KTfwd::check_sum(gametes,30000));
...
BOOST_AUTO_TEST_CASE( low_level_split_demes_test )
{
  ...
  
  {
    diploids = {dipvector_t(1000,std::make_pair(0,0))};
    gametes = {KTfwd::gamete(2000)};
    auto rv = KTfwd::split_deme(rng_ptr,mutations,mcounts,gametes,diploids,0,250,false);
    BOOST_REQUIRE_EQUAL(rv,0);
    BOOST_REQUIRE_EQUAL(diploids.size(),2);
  ...
  //with replacement
  {
    diploids = {dipvector_t(1000,std::make_pair(0,0))};
    gametes = {KTfwd::gamete(2000)};
    auto rv = KTfwd::split_deme(rng_ptr,mutations,mcounts,gametes,diploids,0,990,true);
    BOOST_REQUIRE_EQUAL(rv,0);
    BOOST_REQUIRE_EQUAL(diploids.size(),2);
  ...
  //index out of range
  {
    diploids = {dipvector_t(1000,std::make_pair(0,0))};
    gametes = {KTfwd::gamete(2000)};
    auto rv = KTfwd::split_deme(rng_ptr,mutations,mcounts,gametes,diploids,1,250,false);
    BOOST_REQUIRE_EQUAL(rv,-1);
  }
...
BOOST_AUTO_TEST_CASE( low_level_admix_demes_test )
{
  ...
  
  {
    ...
    gametes={KTfwd::gamete(2000),
	     KTfwd::gamete(2000)};
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,1,0.25,1000,false);
    BOOST_REQUIRE_EQUAL(rv,0);
  ...
  //with resampling
  {
    ...
    gametes={KTfwd::gamete(2000),
	     KTfwd::gamete(2000)};
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,1,0.25,1000,true);
    BOOST_REQUIRE_EQUAL(rv,0);
    BOOST_REQUIRE(KTfwd::check_sum(gametes,6000));
  }
  ...
  //a pop index out of range
  {
    ...
    gametes={KTfwd::gamete(2000),
	     KTfwd::gamete(2000)};
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,2,0.25,1000,false);
    BOOST_REQUIRE_EQUAL(rv,-1);
  }
  ...
  //Fraction of pop i ancestry >= 1.
  {
    ...
    gametes={KTfwd::gamete(2000),
	     KTfwd::gamete(2000)};
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,1,1.,1000,false);
    BOOST_REQUIRE_EQUAL(rv,1);
  }
  ...
  */

  {
    ...
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,1,0.5,2000,false);
    BOOST_REQUIRE_EQUAL(rv,1);
  }
  ...
  //sampling w/replacement has no such limitations
  {
    ...
    //here, we want a new deme that is 50% ancestry from demes 0 and 1.  But, sampling w/o
    //replacement for 2000 individuals will require all individuals from demes i and j.
    auto rv = KTfwd::admix_demes(rng_ptr,mutations,mcounts,gametes,diploids,0,1,0.5,2000,true);
    BOOST_REQUIRE_EQUAL(rv,0);
  }
  gsl_rng_free(rng_ptr);
}
