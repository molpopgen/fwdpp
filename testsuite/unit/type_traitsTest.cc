...
BOOST_AUTO_TEST_CASE( is_mmodel_test )
{
  ...
  auto mmodel = [&next_mut_pos,&i]( decltype(mut_recycling_bin) & rbin,std::vector<mtype> & __mvector )
    {
      //mutations are all neutral
      return KTfwd::fwdpp_internal::recycle_mutation_helper(rbin,__mvector,next_mut_pos[i++], 0. );
    };
  auto v = std::is_convertible<decltype(mmodel),KTfwd::traits::mmodel_t<mcont_t> >::value;
  BOOST_REQUIRE_EQUAL(v,true);

  v=KTfwd::traits::valid_mutation_model<decltype(mmodel),mcont_t,gcont_t>::value;
  BOOST_REQUIRE_EQUAL(v,true);
}
...
BOOST_AUTO_TEST_CASE( is_standard_fitness_model_test )
{
  ...
  auto v = std::is_convertible<decltype(fp),KTfwd::traits::fitness_fxn_t<dipvector_t,gcont_t,mcont_t> >::value;
  BOOST_REQUIRE_EQUAL(v,true);
}
