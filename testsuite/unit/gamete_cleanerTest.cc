...
BOOST_FIXTURE_TEST_SUITE(gamete_cleanerTest, standard_empty_single_deme_fixture)
...
BOOST_AUTO_TEST_CASE( test_remove_all )
{
  ...
  BOOST_REQUIRE_EQUAL( KTfwd::popdata_sane(diploids,gametes,mutations,mcounts), true );
  KTfwd::fwdpp_internal::gamete_cleaner(gametes,mutations,mcounts,2000,std::true_type());
...
BOOST_AUTO_TEST_CASE( test_remove_nothing )
{
  ...
  BOOST_REQUIRE_EQUAL( KTfwd::check_sum(gametes,2000), true );
  BOOST_REQUIRE_EQUAL( KTfwd::popdata_sane(diploids,gametes,mutations,mcounts), true );
  KTfwd::fwdpp_internal::gamete_cleaner(gametes,mutations,mcounts,2000,KTfwd::remove_nothing());
...
BOOST_AUTO_TEST_CASE( test_remove_neutral )
{
  ...
  BOOST_REQUIRE_EQUAL( KTfwd::check_sum(gametes,2000), true );
  BOOST_REQUIRE_EQUAL( KTfwd::popdata_sane(diploids,gametes,mutations,mcounts), true );
  KTfwd::fwdpp_internal::gamete_cleaner(gametes,mutations,mcounts,2000,KTfwd::remove_neutral());
  BOOST_REQUIRE_EQUAL( gametes[0].mutations.size(),0 );
  BOOST_REQUIRE_EQUAL( gametes[0].smutations.size(),1 );
}
