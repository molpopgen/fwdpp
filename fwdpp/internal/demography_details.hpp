...
namespace KTfwd
{
  namespace fwdpp_internal
  {
	...
	{
	  std::size_t ind = std::size_t(gsl_ran_flat(r,0.,double(N)));
	  if(!with_replacement)
	    {
	      while(std::find(rv.begin(),rv.end(),ind)!=rv.end())
		{
		  ind = std::size_t(gsl_ran_flat(r,0.,double(N)));
		}
	    }
	  rv.push_back(ind);
	}
    ...
    int split_deme_replacement( gsl_rng * r,
				...
				const uint_t N_new )
    {
      auto indlist_n = sample_individuals(r,diploids[i].size(),N_new,true);
      auto indlist_c = sample_individuals(r,diploids[i].size(),diploids[i].size()-std::size_t(N_new),true);
    ...
    int split_deme_no_replacement( gsl_rng * r,
				   ...
				   const uint_t N_new )
    {
      auto indlist = sample_individuals(r,diploids[i].size(),N_new,false);
    ...
    {
      return (replacement) ? split_deme_replacement(r,mutations,mcounts,gametes,diploids,i,N_new) :
	split_deme_no_replacement(r,mutations,mcounts,gametes,diploids,i,N_new);
    }
  }
}
