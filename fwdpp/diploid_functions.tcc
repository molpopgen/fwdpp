...
#ifndef NDEBUG
    unsigned NONZERO=0;
    for( unsigned temp = 0 ; temp < max_elements ; ++temp )
      {
	NONZERO += (gcounts[temp] > 0) ? 1 : 0;
      }
    assert( NONZERO > 1 );
#endif
    gsl_ran_discrete_t * preproc = gsl_ran_discrete_preproc (max_elements,gcounts);

    *i = gsl_ran_discrete(r,preproc);
    assert( gcounts[*i] );
    *i = gsl_ran_discrete(r,preproc);
    *j = gsl_ran_discrete(r,preproc);
    assert( gcounts[*j] );
    while(*i==*j ) 
      {
	*j = gsl_ran_discrete(r,preproc);
      }
    gsl_ran_discrete_free(preproc);
  }
 ...
 template< typename gamete_type,
  ...
  unsigned recombine(gsl_rng * r, vector_type<gamete_type,vector_type_allocator > * gametes,
	      ...
	      for(vtype_iterator i=gametes->begin() ;i<gametes->end();++i)
		{
	      ...
	      while(NRECS > 0)
		{
		  //pick_gametes(r,&gcounts[0],ncurrent_classes,&ith,&jth);
	       ...
	       const double * f)
{
  ...
  for( typename diploid_ctr::iterator ptr = diploids->begin() ; ptr != diploids->end() ; ++ptr, ++pop_ptr,++popindex )
    {
      unsigned demesize =*(N_curr+popindex);
      const typename demedips::iterator dptr = ptr->begin(),pptr=(parents.begin()+popindex)->begin();
      typename demedips::iterator pptr2;
      for( unsigned i = 0 ; i < demesize ; ++i )
	{
	  //first parent comes from the current deme
	  size_t p1 = gsl_ran_discrete(r,lookups[popindex]),p2;
	  ...
	  */
	  if( f != NULL && gsl_rng_uniform(r) <= *(f + popindex ) ) //individual is inbred
	    ...
	    {
	      size_t deme_other_parent = mig(popindex);
