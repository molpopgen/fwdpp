...
struct generalmut : public mutation_base {
    ...
    generalmut( array_t __s,
                ...
                double pos,uint_t gen ) : mutation_base(std::move(pos),
                            ...
                            (std::find_if(std::begin(__s),std::end(__s),[](const double d) {
        return d != 0.;
    }) == std::end(__s))),
    s(std::move(__s)),h(std::move(__h)),g(std::move(gen)) {
    }
    bool operator==(const generalmut & rhs) const {
...
/*!
  ...
  This type complements KTfwd::generalmut.  The use cases will be similar.  This type will show better performance
  for large numbers of categories (e.g., large sizes of member variables s and h).

  This type differs from KTfwd::generalmut in that the sizes of the s and h vectors do not have to be the same.
 */
struct generalmut_vec : public mutation_base {
    ...
    bool operator==(const generalmut_vec & rhs) const {
        return this->pos == rhs.pos &&
               this->neutral == rhs.neutral &&
               this->s == rhs.s &&
               this->h == rhs.h;
