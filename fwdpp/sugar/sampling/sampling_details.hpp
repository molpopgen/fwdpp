...
void finish_sample( std::vector<sample_t> & sample, const vec_mutation_t & fixations,
    ...
    for(std::size_t i = 0 ; i < sample.size() ; ++i) {    
		finish_sample(sample[i],fixations,nsam,removeFixed,sugar::treat_neutral::ALL,locus_boundaries[i]);
    }
}
...
void finish_sample( std::vector<sep_sample_t> & sample, const vec_mutation_t & fixations,
					...
					const std::vector<std::pair<double,double> > & locus_boundaries) {
    for(std::size_t i=0;i<sample.size();++i){    
		finish_sample(sample[i].first,fixations,nsam,removeFixed,sugar::treat_neutral::NEUTRAL,locus_boundaries[i]);
        finish_sample(sample[i].second,fixations,nsam,removeFixed,sugar::treat_neutral::SELECTED,locus_boundaries[i]);
    }
}
