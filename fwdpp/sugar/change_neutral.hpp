...
namespace KTfwd
{
  namespace sugar
  {
    template<typename mcont_t,
	     typename mut_key_cont_t>
    void change_neutral_details(const mcont_t & mutations,
				const double pos,
  ...
  /*!
    ...
    This function changes mutation_base::neutral to !mutation_base::neutral at position 
    mindex and updates the storage of this mutation in all gametes.

    \note This function does not change any other member data at p.mutations[mindex]!!  Thus, if you change a mutation
    from "neutral" to "selected", you must manually change the relevant member data to reflect its
    new effect size.  For the simplest use case of making a mutation no longer subject to selection,
    no additional changes are needed.  Rather, it is sufficient that mutation_base::neutral == true.
  ...
  */
  {
    if(mindex>=p.mutations.size()) throw std::out_of_range("mindex >= p.mutations.size()");
	...
	if(n)
	  {
	    sugar::change_neutral_details(p.mutations,pos,mindex,g.mutations,g.smutations);
	  }
	else
	  {
	    sugar::change_neutral_details(p.mutations,pos,mindex,g.smutations,g.mutations);
	  }
