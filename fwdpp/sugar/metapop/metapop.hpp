...
namespace KTfwd {
  namespace sugar {
    ...
    */
    ...
    class metapop : public popbase<mutation_type,mcont,gcont,dipvector,mvector,ftvector,lookup_table_type>
    {
    private:
      void init_vectors()
      {
	for(uint_t i = 0 ; i < Ns.size() ; ++i )
	  {
	    diploids.emplace_back( typename popbase_t::dipvector_t(Ns[i],typename dipvector::value_type(0,0)) );
	  }
      }
    public:
      //! Deme sizes
      ...
      using compat_singlepop_t = sugar::singlepop<typename popbase_t::mutation_t,
						  typename popbase_t::mcont_t,
						  typename popbase_t::gcont_t,
						  typename popbase_t::dipvector_t,
						  typename popbase_t::mvector_t,
						  typename popbase_t::ftvector_t,
	...
	this->fixation_times=spop.fixation_times;
      ...
      metapop( compat_singlepop_t && spop ) : popbase_t(0),
					      Ns({spop.N})
      {
