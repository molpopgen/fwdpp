				   ...
				   const double & sigma )
{
  ...
  if(u <= mu_male/mu_total)
    {
      return KTfwd::fwdpp_internal::recycle_mutation_helper(mut_recycling_bin,mutations,pos,gsl_ran_gaussian(r,sigma),false,false);
    }
  else if (u <= (mu_male+mu_female)/mu_total)
    {
      return KTfwd::fwdpp_internal::recycle_mutation_helper(mut_recycling_bin,mutations,pos,gsl_ran_gaussian(r,sigma),true,false);
    }
  ...
  //smutations
  return KTfwd::fwdpp_internal::recycle_mutation_helper(mut_recycling_bin,mutations,pos,gsl_ran_gaussian(r,sigma),false,true);
}
...
struct sexSpecificRules
{
	 ...
	 const fitness_func & ff)const
  {
	...
	w = KTfwd::fwdpp_internal::diploid_fitness_dispatch(ff,diploids[i],gametes,mutations,
			     ...
			     gsl_rng * r, const double & sigmaE )
{
  double trait_value = std::accumulate(gametes[dip.first].smutations.begin(),
				       ...
				       0.,[&dip,&mutations](const double a, const std::size_t & m)
					{
					  return a + ((dip.sex==mutations[m].sex) ? mutations[m].s : 0.);
					} );
  trait_value += std::accumulate(gametes[dip.second].smutations.begin(),
				 gametes[dip.second].smutations.end(),
				 0.,[&dip,&mutations](const double a, const std::size_t & m)
				 {
				   return a + ((dip.sex==mutations[m].sex) ? mutations[m].s : 0.);
				 } );
      ...
      for( unsigned generation = 0 ; generation < ngens ; ++generation )
	{
	  ...
	  double wbar = KTfwd::experimental::sample_diploid(rng.get(),
							    ...
							    std::bind(sex_specific_mut_model,std::placeholders::_1,std::placeholders::_2,rng.get(),
								      std::ref(pop.mut_lookup),mu_total,mu_male,mu_female,sigma),
							    std::bind(KTfwd::poisson_xover(),rng.get(),recrate,0.,1.,
								      std::placeholders::_1,std::placeholders::_2,std::placeholders::_3),
							    std::bind(sex_specific_fitness,std::placeholders::_1,
								      std::placeholders::_2,std::placeholders::_3,rng.get(),sigmaE),
      ...
      std::pair< std::vector< std::pair<double,std::string> >,
		 std::vector< std::pair<double,std::string> > > sample = KTfwd::ms_sample_separate(rng.get(),pop.mutations,pop.gametes,pop.diploids,samplesize1);
